...
## 트러블 슈팅
<!-- ### 트러블 슈팅 1 -->
<!-- - 문제 : 무통장 입금 기반 주문은 상품 가격, 옵션 추가금, 배송비, 입금 기한, 주문 상태가 여러 테이블에 걸쳐 함께 변경되어야 해서 일부 데이터만 저장되면 운영자가 주문을 신뢰하기 어려웠습니다.
- 해결 : 주문 생성 시 주문, 주문 상품, 옵션 스냅샷, 연락처, 입금 정보를 하나의 트랜잭션으로 저장하고, 가격 계산과 입금 기한 계산을 도메인 함수로 분리해 단위 테스트로 검증했습니다.
- 결과 : 주문번호 생성부터 입금 대기 상태까지 일관된 주문 데이터가 생성되며, 상품 정보가 이후 수정되어도 주문 당시의 스냅샷을 기준으로 운영할 수 있게 되었습니다. -->
